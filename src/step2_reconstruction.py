...
CONFIDENCE_THRESH = 0.0  # ### CHANGED: stricter confidence filter
...
BBOX_EXPANSION = 1.05  # ### CHANGED: crop Poisson mesh to slightly expanded AABB of point cloud
POISSON_DEPTH = 10      # you can try 8–10 depending on scene
...
def depth_to_pointcloud(depth_map, color_img, intrinsic_matrix, confidence_map=None):
    """
    Back-project depth map to a colored point cloud using pinhole intrinsics.
    """
...
def fuse_pointclouds(scan_dir: str, output_dir: str):
    ...
    pcd_clean, inlier_idx = pcd_down.remove_statistical_outlier(
        nb_neighbors=20,
        std_ratio=1.5  # ### CHANGED: slightly stricter
    )
    print(f"Points after statistical filtering: {len(pcd_clean.points):,}")

    # ### CHANGED: radius outlier removal for isolated points
    print("Removing radius-based outliers...")
    pcd_clean, inlier_idx = pcd_clean.remove_radius_outlier(
        nb_points=16,
        radius=0.08
    )
    print(f"Points after radius filtering: {len(pcd_clean.points):,}")
    ...
    # Estimate normals (needed for plane detection & Poisson)
...
def generate_mesh(pcd, output_dir: str):
    ...
    # Remove low-density vertices (noise) BEFORE cropping
    print("Filtering low-density vertices...")
    densities = np.asarray(densities)
    density_threshold = np.quantile(densities, DENSITY_FILTER_QUANTILE)
    vertices_to_remove = densities < density_threshold
    mesh.remove_vertices_by_mask(vertices_to_remove)

    # Crop Poisson mesh to bounding box
    bbox = pcd.get_axis_aligned_bounding_box()
    bbox = bbox.scale(BBOX_EXPANSION, bbox.get_center())
    mesh = mesh.crop(bbox)

    # ### REMOVED: fill_holes is not available in legacy TriangleMesh API
...
def generate_floor_plan(pcd, planes, output_dir: str):
    ...
    # Find floor (plane with most upward-facing normal and lowest Z-like distance)
    floor_plane = None
    floor_z = None
    ...
    for plane in planes:
        ...
        # Upward-ish normals
        if normal[2] > 0.8:
            # Approximate signed distance to origin using plane equation
            a, b, c, d = plane['equation']
            # Distance of plane from origin along its normal is |d| / ||n||
    ...
    if floor_plane is None:
        print("⚠️ Warning: Could not identify floor plane, using min Z of points")
    ...
    if len(slice_points) < 100:
        print("⚠️ Warning: Very few points at slice height, adjusting to median Z band...")
...
def run_step2(scan_dir: str,
              ...
              generate_mesh_flag: bool = True) -> None:
